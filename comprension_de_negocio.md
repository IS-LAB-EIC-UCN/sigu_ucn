...
5. **Separación de Privilegios (RBAC):** El sistema debe distinguir estrictamente entre administradores y usuarios en la capa de controladores (backend) y vistas (frontend). Las rutas de administración (creación/cancelación de eventos y mantenedor de expositores) deben bloquear peticiones de usuarios sin rol 'ADMIN' retornando un error HTTP 403. Las plantillas JTE deben ocultar condicionalmente las herramientas administrativas al evaluar el rol del usuario en sesión.
